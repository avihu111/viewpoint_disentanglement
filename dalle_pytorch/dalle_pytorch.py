...
class DALLE(nn.Module):
    ...
    def generate_images(
        ...
        for cur_len in range(image_seq_len):
            logits = self(image=out[:cur_len + text_seq_len], frame_indices=frame_index,
                          video_indices=video_index, sigma=0)[:, cur_len + text_seq_len - 1, :]
    ...
    def forward(
        ...
        noisy_embs = frame_embs + torch.normal(0, 1, size=frame_embs.shape).cuda(device) * sigma
        image, logits = self.get_logits(image, noisy_embs, rel_codes, video_embs)
        if not return_loss:
            return logits
        labels = image
        ...
        dim_size = frame_embs.shape[-1]

        frame_emb_loss = (frame_embs ** 2).mean() * dim_size
        return loss_img, self.frame_emb_regularization * frame_emb_loss

    def get_logits(self, image, frame_embs, rel_codes, video_embs):
        device = image.device
        bos_index = torch.full(size=(len(frame_embs),), fill_value=self.per_frame_emb.num_embeddings - 1,
                               dtype=torch.long)
        bos_embs = self.per_frame_emb(bos_index.cuda(device)).unsqueeze(1)
        tokens = torch.cat([bos_embs, video_embs, frame_embs], dim=1).cuda(device)
